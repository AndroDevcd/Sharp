...
void compile_dictionary_expression(expression *e, Ast *ast) {
    ...
    for(Int i = inferDictType ? 0 : 1; i < ast->getSubAstCount(); i++) {
        if(!is_implicit_type_match(key, dictionaryItems.at(i).key->type,
            ...
            currThread->currTask->file->errors->createNewError(GENERIC, ast->line, ast->col,
                     " expected dictionary key of type `" + type_to_str(key) +
                     "` but found `" + type_to_str(dictionaryItems.at(i).key->type) + "`.");
        }

        if(!is_implicit_type_match(value, dictionaryItems.at(i).value->type,
                                   constructor_only)) {
            currThread->currTask->file->errors->createNewError(GENERIC, ast->line, ast->col,
                      " expected dictionary value of type `" + type_to_str(value) +
                            "` but found `" + type_to_str(dictionaryItems.at(i).value->type) + "`.");
        }
    }
    ...
    sharp_class *dictionaryClassGeneric = resolve_class(get_module(stdModule), "dictionary", true, false), *dictionaryClass = NULL;
    ...
    if(pairClassGeneric != NULL && dictionaryClassGeneric != NULL) {
        ...
        if(pairClass != NULL && dictionaryClass != NULL) {
            ...
            if(constructor != NULL) {
                ...
                currThread->currTask->file->errors->createNewError(GENERIC, ast->line, ast->col,
                                                                   "cannot find constructor `" + type_to_str(unresolvedType) +"` for class `" + dictionaryClass->fullName + "`.");
            }
        } else {
