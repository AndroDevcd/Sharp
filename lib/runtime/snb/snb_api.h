...
#define use_var(a, code) \
    a.refresh(); \
    { code }

#define use_vars(a, b, code) \
    (a).refresh(); \
    (b).refresh(); \
    { code }
...
typedef void (*_return_object)(SharpObject obj);
typedef void (*_return_number)(double num);
typedef void (*_return_call)();
typedef bool (*_validate_app_id)(int id);
...
namespace snb_api {
    ...
    void check_for_err();
    void check_app_id(int applicationId, void (*stringConstructor)(SharpObject instance, SharpObject message));
