...
bool Param::match(List<Param>& p1, List<Param>& p2, bool nativeSupport, bool ambiguousProtect, bool find) {
    if(p1.size() != p2.size())
        return false;
    ...
    for(unsigned int i = 0; i < p1.size(); i++) {
        if(!p1.get(i).match(p2.get(iter++))) {
            if((find && p1.get(i).field.type == OBJECT && p1.get(i).field.type == OBJECT) || (!ambiguousProtect && p1.get(i).field.type == CLASS
