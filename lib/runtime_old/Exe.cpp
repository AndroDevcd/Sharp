...
int Process_Exe(std::string &exe)
{
    ...
    for(Int i = 0; i < nativeSymbolCount; i++) {
        ...
        vm.nativeSymbols[i].type = (data_type)i;
    }
                ...
                if(klass->totalFieldCount != 0) {
                    for( ;; ) {
                        if(buffer.at(currentPos) == data_field) {
                            ...
                            field->type = (data_type) geti32(buffer);
...
Symbol* getSymbol(File::buffer& buffer) {
    data_type type = (data_type)geti32(buffer);
    if((type >= type_int8 && type <= type_uint64) // todo this can be streacted out to a function in the "core" direcctory
