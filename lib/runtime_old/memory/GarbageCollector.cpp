        ...
        if(size > 0) {
            ...
            if(autoInit) {
                ...
                for (unsigned int i = 0; i < object->size; i++) {
                     ...
                     */
                    if (k->fields[fieldAddress].type <= type_var && !k->fields[fieldAddress].isArray) {
                        if (!staticInit || (staticInit && IS_STATIC(k->fields[fieldAddress].flags))) {
                            object->node[i] = newObject(1, k->type < type_function_ptr ? k->type : NTYPE_VAR);
                        }
                    }
            ...
            if (size > 0) {
                ...
                if (object->node != NULL) {
                    ...
                    for (unsigned int i = 0; i < object->size; i++) {
                        ...
                        if (k->fields[fieldAddress].type <= type_var && !k->fields[fieldAddress].isArray) {
                            if (!staticInit || (staticInit && IS_STATIC(k->fields[fieldAddress].flags))) {
                                object->node[i] = newObjectUnsafe(1, k->type < type_function_ptr ? k->type : NTYPE_VAR);
                            }
                        }
...
void GarbageCollector::createStringArray(Object *object, string& str) {
    if(object != nullptr) {
        *object = newObject(str.length(), type_int8);
