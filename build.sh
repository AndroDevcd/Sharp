...
build_compiler() {
    ...
    g++ -o bin/sharpc -std=c++11 -pthread -lstdc++ -O3 -Ofast -g main.cpp stdimports.h -DMAKE_COMPILER \
...
build_runtime() {
    ...
    g++ -o bin/sharp -std=c++11 -pthread -lstdc++ -Wl,--no-as-needed -lm -lrt -lstdc++ -O3 -ldl -mno-red-zone -g -Ofast main.cpp stdimports.h \
...
install_files() {
    ...
    if [ -f "bin/sharpc" ]; then
        ...
        echo "Sharp compiler installed to /bin/sharpc"
    fi
    
    if [ -f "bin/sharp" ]; then
        sudo cp bin/sharp /bin
        echo "Sharp runtime installed to /bin/sharp"
    fi
    
    # Install standard library
    echo "Installing standard library..."
    sudo rm -rf /usr/include/sharp/
    sudo mkdir -p /usr/include/sharp/
    sudo cp -r lib/support/0.3.0/ /usr/include/sharp/
